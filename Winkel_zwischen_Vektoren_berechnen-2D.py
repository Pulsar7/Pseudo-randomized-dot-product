...
r = (-2,3) #Hier können die Werte beliebig geändert werden! (X,Y)
s = (5,-7) #Hier können die Werte beliebig geändert werden! (X,Y)

skalar_produkt = (r[0] * s[0] + (r[1]) * s[1])
betrag_r = ((r[0])**2 + (r[1])**2)
betrag_s = ((s[0])**2 + (s[1])**2)
